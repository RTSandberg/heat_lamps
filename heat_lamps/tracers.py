...
def add_tracers(self, xs_tracers, vs_tracers, dt):
    ...
    Es_tracers = self.calc_E(self.xs_tracers, self.x0s,self.weights, self.L, self.delta)
    
    self.vs_old_tracers -= .5 * dt * self.qm * Es_tracers
    self.vs_new_tracers += .5 * dt * self.qm * Es_tracers
...
def update(self, dt):
    ...
    self.vs_new_tracers += dt * self.qm * self.calc_E(self.xs_tracers, self.xs, self.weights, self.L,self.delta)
